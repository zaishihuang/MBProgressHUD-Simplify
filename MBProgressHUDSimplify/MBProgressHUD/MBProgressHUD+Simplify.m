...
{
    UIImageView *customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"MB-Checkmark"]];
...
{
    UIImageView *customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"MB-Error.png"]];
    [self showInView:view message:message customView:customView duration:duration];
}
...
- (void)showMessage:(NSString *)message mode:(MBProgressHUDMode)mode duration:(NSTimeInterval)duration
{
    ...
    if (duration >= 0) [self hide:YES afterDelay:duration];
}
